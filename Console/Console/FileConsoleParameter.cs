...
/// <summary>
/// Implements a type of console parameter that parses a FileInfo
/// </summary>
public class FileConsoleParameter : INamedConsoleParameter
{
    private readonly Action<FileInfo> action;
    ...
    /// <summary>
    /// Instantiates a new instance of a FileConsoleParameter
    /// </summary>
    ...
    public FileConsoleParameter(string[] ids, Action<FileInfo> action, string description, bool isRequired = false)
    {
        Ids = ids;
        this.action = action;
    ...
    public bool PerformMapping(LinkedList<string> args)
    {
        ...
        foreach (var key in keys)
        {
            ...
            {
                action(new FileInfo(node.Next.Value));
            }
            else
            {
                action(new FileInfo(Path.Combine(Environment.CurrentDirectory, node.Next.Value)));
            }
